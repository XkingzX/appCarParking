  ...
  Future<void> _loadActiveBookings() async {
    ...
    try {
      ...
      for (final booking in bookings) {
        if (booking.parkingLotId != null &&
            !_priceCache.containsKey(booking.parkingLotId)) {
  ...
  Future<void> _cancelBooking(BookingModel booking) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Bạn có chắc muốn hủy đặt chỗ tại ${booking.parkingLotName ?? 'bãi đỗ'}?'),
        actions: [
          TextButton(
  ...
  Widget _buildCurrentParkingTab() {
    ...
    if (_activeBookings.isEmpty) {
      return _buildEmptyState(
        ...
        title: 'Không có đặt chỗ nào đang hoạt động',
        subtitle: 'Các lần đặt chỗ đang đỗ sẽ hiện tại đây.',
      );
    }
  ...
  Widget _buildHistoryTab() {
    ...
    if (_historyBookings.isEmpty) {
      return _buildEmptyState(
        ...
        title: 'Chưa có lịch sử đỗ xe',
        subtitle: 'Lịch sử đỗ xe sẽ được hiển thị tại đây.',
      );
    }
